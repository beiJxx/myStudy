...
public class DataTableUtil {
	...
	static String[] Propertys = { "地块编号", "宗地编号", "地块位置", "用地性质", "规划面积",
			...
			"成交价", "交易方式", "竞得人" };
	static String[] bankProperties = {"序号","行号","网点名称","电话","地址"};
	...
	public static List<String> getBankElement(Document document){
		List<String> bankValueElement = new ArrayList<String>();
		Element element = document.getElementsByTag("body").get(0);
		List<TableElement> fitElments = new ArrayList<TableElement>();
		Elements tableElements = element.getElementsByTag("table");
		if (tableElements != null && tableElements.size() > 0) {
			for (int i = 0; i < tableElements.size(); i++) {
				...
				if (ces != null && ces.size() > 1) {
					...
					bankValueElement = isBankValueElement(bankProperties, tableElement);
				}
			}
		}
		return bankValueElement;
	}
	...
	private static List<String> isBankValueElement(String[] bankProperties,Element element){
		...
		if (trElements != null && trElements.size() > 0) {
			for (Element thelement : trElements) {
				propertyElements.add(thelement);
			}
		}
		/*if (tdElements != null && tdElements.size() > 0) {
		...
		for (Element propertyElement : propertyElements) {
			String text = propertyElement.text();
			if(text.equals("序号 行号 网点名称 电话 地址")){
				start = true;
			}
			if(start){
				String[] split = text.split(" ");
				if(!"行号".equals(split[1])){
